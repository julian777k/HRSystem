...
export async function POST(request: NextRequest) {
  try {
    ...
    // D1 파라미터 한도(100) 때문에 IN 조회는 ID를 나눠 실행한다.
    const employeeIds = employees.map((e) => e.id);

    // Batch load all existing grants for the year (N+1 해소)
    const allExistingGrants = await findInChunks(employeeIds, (ids) =>
    ...
    const existingBalances = await findInChunks(employeeIds, (ids) =>
    ...
    const existingWallets = await findInChunks(employeeIds, (ids) =>
